...
* David added an else clause to the simfit() function to return fit for model treating coach as fixed effect when method = 'lmer_fixed', and added 'Coach as fixed effect' section to end of code, calling simfit() with method = 'lmer_fixed'.
* David added a note to 'Coach as fixed effect' section, about how the outcome value is generated for arm 3.
...
variance_total_no_trt <- variance_village + variance_residual # Not used, but included here for clarity
  ...
  # simulated outcome data
    mutate(y = b_0 +
               b_ind_asset*coaching_individual +  #shouldn't apply to arm 3, right?
...
if (action == "fit"){
  ...
  # fit by method
    ...
    } else if (method == "lmer_fixed") {
      
      # data set for fixed effects analysis of coaches
      # assign "none" as coach for control arms
      # set "none" as reference level
  
      df_fixed <- df %>%
        mutate(coach = factor(ifelse(trt == 0, "none", coach))) %>%
        mutate(coach = relevel(coach, ref = "none"))
    ...
    # results 
      res <- broom.mixed::tidy(fit, conf.int = TRUE, conf.level = ci) %>%
        mutate(type = "raw") %>%
        filter(str_detect(term, "coach")) %>%
        mutate(coach = str_remove(term, "coach")) %>%
        left_join(df %>% select(coach, arm) %>% unique(), by = "coach")
...
```{r}
  ...
  df %>% filter(no_asset == 1) %>% select(no_asset, coaching_individual, arm) %>% unique()
```
```{r}
  df %>% 
...
## Coach as fixed effect

Note: Treating coach as a fixed effect, terms for arms 1-3 can't also be included in the model. The coefficients for coaches account for any effect of asset vs no asset, and for group coaching vs individual coaching group. Therefore I group the summaries of the coefficients below by arm.

Looking at the boxplots grouped by arm (below), I noticed the coefficients for arm 3 (individual coaching, no asset) were higher than for arms 1 and 2. Looking back at how the outcome data is generated in simfit (lines 411-422 in this file), arm 3 observations have the value 1 for the coaching_individual indicator and also for the no_asset indicator, so they get a total treatment coefficient of b_ind_asset + b_ind_noasset. The code generating the outcome data could be modified using ifelse() or case_when(), so that only the b_ind_noasset input contributes to arm 3.
```{r}
  m2 <- simfit(seed = 8675309, 
               action = "fit",
               method = "lmer_fixed",
               ci = 0.95,
               k_villages = 114,
...
coach_fixed_coefficients <- m2[[2]] %>%
  arrange(desc(estimate))

coach_fixed_coefficients %>%
  select(coach, arm, estimate, contains("conf")) %>%
  print(n = 20)

# coach_fixed_coefficients %>%
#   ggplot(aes(x = estimate)) +
#   geom_histogram() +
#   labs(title = "Histogram of coefficients treating coach as fixed effect, \nwith 'no coach' as reference level",
#        x = "coefficient estimate")

coach_fixed_coefficients %>%
  group_by(arm) %>%
  summarize(min = min(estimate), Q1 = quantile(estimate, 0.25), 
            median = median(estimate), Q3 = quantile(estimate, 0.75),
            max = max(estimate)) %>%
  kable(caption = "Summary of estimated coefficients for coach as fixed effect",
        digits = 2)

coach_fixed_coefficients %>% mutate(arm = as_factor(arm)) %>%
  ggplot(aes(x = estimate, y = arm)) +
  geom_boxplot() +
  labs(title = "Boxplots of estimated coefficients treating coach as fixed effect, \nwith 'no coach' as reference level",
       x = "coefficient estimate")
